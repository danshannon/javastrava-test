...
public interface SegmentAPI {
	...
	/**
	 * @param id
	 *            The id of the athlete whose starred segments are to be
	 *            retrieved
	 * @param page
	 *            (optional) Page number to be returned
	 * @param perPage
	...
	public Response listStarredSegments(@Path("id") final Integer id, @Query("page") final Integer page, @Query("per_page") final Integer perPage) throws NotFoundException;

	/**
	 * @see javastrava.api.v3.service.SegmentService#listSegmentEfforts(Integer,
	 *      Integer, LocalDateTime, LocalDateTime, javastrava.util.Paging)
	 *
	 * @param id
	 *            The id of the {@link StravaSegment} for which
	 *            {@link StravaSegmentEffort segment efforts} are to be returned
	 * @param athleteId
	 *            (Optional) id of the {@link StravaAthlete} to filter results
	 *            by
	 * @param start
	 *            (Optional) ISO 8601 formatted date time
	 * @param end
	 *            (Optional) ISO 8601 formatted date time
	 * @return Returns an array of {@link StravaSegmentEffort segment effort}
	 *         summary {@link StravaResourceState representations} sorted by
	...
	public Response listSegmentEfforts(@Path("id") final Integer id, @Query("athlete_id") final Integer athleteId, @Query("start_date_local") final String start,
			@Query("end_date_local") final String end, @Query("page") final Integer page, @Query("per_page") final Integer perPage) throws NotFoundException;
	 ...
	 *      StravaLeaderboardDateRange, javastrava.util.Paging, Integer)
	 *
	 * @param id
	 *            Segment identifier
	 * @param gender
	 ...
	 * @param following
	 *            (Optional) If <code>true</code> then filter leaderboard by
	 *            athletes the authenticated athlete is following
	 * @param clubId
	 *            (Optional) Club to filter the leaderboard by
	 * @param dateRange
	 *            (Optional) Date range (this year, this month etc.) to filter
	 *            the leaderboard by
	 * @param page
	 ...
	 *            (Optional) Number of context entries to return either side of
	 *            the authenticated athlete (default is 2, maximum is 15)
	 * @return A Strava leaderboard
	 * @throws NotFoundException
	 *             If the segment with the given id doesn't exist
	 */
	@GET("/segments/{id}/leaderboard")
	public Response getSegmentLeaderboard(@Path("id") final Integer id, @Query("gender") final StravaGender gender, @Query("age_group") final StravaAgeGroup ageGroup,
			...
			@Query("context_entries") final Integer contextEntries) throws NotFoundException;

	/**
	 * @see javastrava.api.v3.service.SegmentService#segmentExplore(javastrava.api.v3.model.StravaMapPoint,
	 *      javastrava.api.v3.model.StravaMapPoint,
	 *      StravaSegmentExplorerActivityType, StravaClimbCategory,
	 *      StravaClimbCategory)
	 *
	 * @param bounds
	 *            Pair of co-ordinates defining a box which is to be searched
	 *            for segments
	 * @param activityType
	 *            (Optional) Activity type to filter by (default is 'ride')
	 * @param minCategory
	 *            (Optional) Minimum climb category for which rides should be
	 *            returned
	 * @param maxCategory
	 *            (Optional) Maximum climb category for which rides should be
	 *            returned
