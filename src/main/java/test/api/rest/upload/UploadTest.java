...
public class UploadTest extends APITest<StravaUploadResponse> {
	...
	public void testUpload_badActivityType() throws Exception {
		RateLimitedTestRunner.run(() -> {
			...
			final StravaUploadResponse response = apiWithWriteAccess().upload(StravaActivityType.UNKNOWN, "UploadServicesImplTest,testUpload_badActivityType", null, null, null, null, "gpx", "ABC",
					typedFile);
	...
	public void testUpload_badDataType() throws Exception {
		RateLimitedTestRunner.run(() -> {
			...
			try {
				response = apiWithWriteAccess().upload(StravaActivityType.RIDE, "UploadServicesImplTest.testUpload_badDataType", null, null, null, null, "UNKNOWN", "ABC", typedFile);
	...
	public void testUpload_badFileContent() throws Exception {
		RateLimitedTestRunner.run(() -> {
			...
			final TypedFile typedFile = new TypedFile("text/xml", file);
			final StravaUploadResponse response = apiWithWriteAccess().upload(StravaActivityType.RIDE, "UploadServicesImplTest.testUpload_noName", null, null, null, null, "gpx", "ABC", typedFile);
	...
	public void testUpload_noFile() throws Exception {
		RateLimitedTestRunner.run(() -> {
			StravaUploadResponse response = null;
			try {
				response = apiWithWriteAccess().upload(StravaActivityType.RIDE, "UploadServicesImplTest.testUpload_noName", null, null, null, null, "gpx", "ABC", null);
			} catch (final BadRequestException e) {
				// Expected
				return;
			}
	...
	public void testUpload_noName() throws Exception {
		RateLimitedTestRunner.run(() -> {
			final File file = new File("hyperdrive.gpx");
			final TypedFile typedFile = new TypedFile("text/xml", file);
			final StravaUploadResponse response = apiWithWriteAccess().upload(StravaActivityType.RIDE, null, "UploadServicesImplTest.testUpload_noName", null, null, null, "gpx", "ABC", typedFile);
			waitForCompletionAndDelete(response);
		});
	}
	...
	public void testUpload_noWriteAccess() throws Exception {
		RateLimitedTestRunner.run(() -> {
			...
			StravaUploadResponse response = null;
			try {
				response = api().upload(StravaActivityType.RIDE, "UploadServicesImplTest.testUpoad_noWriteAccess", null, Boolean.TRUE, null, null, "gpx", "testUpload_noWriteAccess", typedFile);
	...
	public void testUpload_valid() throws Exception {
		RateLimitedTestRunner.run(() -> {
			final File file = new File("hyperdrive.gpx");
			final TypedFile typedFile = new TypedFile("text/xml", file);
			final StravaUploadResponse response = apiWithWriteAccess().upload(StravaActivityType.RIDE, "UploadServicesImplTest", null, null, null, null, "gpx", "ABC", typedFile);
			waitForCompletionAndDelete(response);
		});
	}
