...
public class UpdateActivityAsyncTest extends AsyncAPITest {
	...
	private StravaActivity createUpdateAndDelete(final StravaActivity activity, final StravaActivityUpdate update) throws Exception {
		final StravaActivity response = apiWithFullAccess().createManualActivity(activity).get();
		StravaActivity updateResponse = null;
		try {
			updateResponse = apiWithFullAccess().updateActivity(response.getId(), update).get();
	...
	private StravaActivity waitForUpdateCompletion(final StravaActivity updateResponse) throws Exception {
		...
		while (i < 600) {
			response = apiWithFullAccess().getActivity(updateResponse.getId(), null).get();
			i++;
	...
	public void testUpdateActivity_accessTokenDoesNotHaveWriteAccess() throws Exception {
		RateLimitedTestRunner.run(() -> {
			...
			try {
				api().updateActivity(activity.getId(), new StravaActivityUpdate(activity)).get();
	...
	public void testUpdateActivity_invalidActivity() throws Exception {
		RateLimitedTestRunner.run(() -> {
			...
			StravaActivity response = null;
			try {
				response = apiWithWriteAccess().updateActivity(activity.getId(), new StravaActivityUpdate(activity)).get();
	...
	public void testUpdateActivity_nullUpdate() throws Exception {
		RateLimitedTestRunner.run(() -> {
			try {
				apiWithWriteAccess().updateActivity(TestUtils.ACTIVITY_FOR_AUTHENTICATED_USER, null).get();
	...
	public void testUpdateActivity_unauthenticatedAthletesActivity() throws Exception {
		RateLimitedTestRunner.run(() -> {
			final StravaActivity activity = apiWithWriteAccess().getActivity(TestUtils.ACTIVITY_FOR_UNAUTHENTICATED_USER, null).get();

			try {
				apiWithWriteAccess().updateActivity(activity.getId(), new StravaActivityUpdate(activity)).get();
			} catch (final UnauthorizedException e) {
				// Expected behaviour
				return;
			}
	...
	public void testUpdateActivity_validUpdatePrivateNoViewPrivate() throws Exception {
		RateLimitedTestRunner.run(() -> {
			...
			// Create the activity
			StravaActivity response = apiWithFullAccess().createManualActivity(activity).get();
			...
			try {
				response = apiWithWriteAccess().updateActivity(response.getId(), new StravaActivityUpdate(activity)).get();
			} catch (final UnauthorizedException e) {
				// expected
				forceDeleteActivity(response);
				return;
