...
public class ListActivityZonesAsyncTest extends AsyncAPITest {
	/**
	 * <p>
	...
	public void testListActivityZones_hasNoZones() throws Exception {
		RateLimitedTestRunner.run(() -> {
			final StravaActivityZone[] zones = api().listActivityZones(TestUtils.ACTIVITY_WITHOUT_ZONES).get();
	...
	public void testListActivityZones_hasZones() throws Exception {
		RateLimitedTestRunner.run(() -> {
			final StravaActivityZone[] zones = api().listActivityZones(TestUtils.ACTIVITY_WITH_ZONES).get();
	...
	public void testListActivityZones_invalidActivity() throws Exception {
		RateLimitedTestRunner.run(() -> {
			try {
				api().listActivityZones(TestUtils.ACTIVITY_INVALID).get();
	...
	public void testListActivityZones_privateActivity() throws Exception {
		RateLimitedTestRunner.run(() -> {
			try {
				api().listActivityZones(TestUtils.ACTIVITY_PRIVATE_OTHER_USER).get();
	...
	public void testListActivityZones_privateActivityWithViewPrivate() throws Exception {
		RateLimitedTestRunner.run(() -> {
			final StravaActivityZone[] zones = apiWithViewPrivate().listActivityZones(TestUtils.ACTIVITY_PRIVATE_WITH_PHOTOS).get();
			assertNotNull(zones);
	...
	public void testListActivityZones_privateActivityWithoutViewPrivate() throws Exception {
		RateLimitedTestRunner.run(() -> {
			try {
				api().listActivityZones(TestUtils.ACTIVITY_PRIVATE_WITH_PHOTOS).get();
			} catch (final UnauthorizedException e) {
				// Expected
				return;
			}
