...
public class ListActivityKudoersAsyncTest extends PagingArrayMethodAsyncTest<StravaAthlete, Integer> {
	@Override
	protected ArrayCallback<StravaAthlete> callback() {
		return (paging -> api().listActivityKudoers(TestUtils.ACTIVITY_WITH_KUDOS, paging.getPage(), paging.getPageSize()).get());
	}
	...
	public void testListActivityKudoers_hasKudoers() throws Exception {
		RateLimitedTestRunner.run(() -> {
			final StravaAthlete[] kudoers = api().listActivityKudoers(TestUtils.ACTIVITY_WITH_KUDOS, null, null).get();
	...
	public void testListActivityKudoers_hasNoKudoers() throws Exception {
		RateLimitedTestRunner.run(() -> {
			final StravaAthlete[] kudoers = api().listActivityKudoers(TestUtils.ACTIVITY_WITHOUT_KUDOS, null, null).get();
	...
	public void testListActivityKudoers_invalidActivity() throws Exception {
		RateLimitedTestRunner.run(() -> {
			try {
				api().listActivityKudoers(TestUtils.ACTIVITY_INVALID, null, null).get();
	...
	public void testListActivityKudoers_privateActivity() throws Exception {
		RateLimitedTestRunner.run(() -> {
			try {
				api().listActivityKudoers(TestUtils.ACTIVITY_PRIVATE_OTHER_USER, null, null).get();
	...
	public void testListActivityKudoers_privateActivityWithViewPrivate() throws Exception {
		RateLimitedTestRunner.run(() -> {
			final StravaAthlete[] kudoers = apiWithViewPrivate().listActivityKudoers(TestUtils.ACTIVITY_PRIVATE_WITH_KUDOS, null, null).get();
