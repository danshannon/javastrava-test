...
public class CreateManualActivityAsyncTest extends StravaTest {
	/**
	 * <p>
	...
	public void testCreateManualActivity_accessTokenDoesNotHaveWriteAccess() throws Exception {
		RateLimitedTestRunner.run(() -> {
			...
			try {
				...
				response = strava().createManualActivityAsync(activity).get();
	...
	public void testCreateManualActivity_invalidType() throws Exception {
		RateLimitedTestRunner.run(() -> {
			...
			try {
				stravaResponse = stravaWithWriteAccess().createManualActivityAsync(activity).get();
	...
	public void testCreateManualActivity_validActivity() throws Exception {
		RateLimitedTestRunner.run(() -> {
			...
			final StravaActivity response = stravaWithWriteAccess().createManualActivityAsync(activity).get();
