...
public class GetActivityAsyncTest extends StravaTest {
	 ...
	 */
	@Test
	public void testGetActivityAsync_knownActivityBelongsToAuthenticatedUser() throws Exception {
		RateLimitedTestRunner.run(() -> {
			final CompletableFuture<StravaActivity> future = strava()
					.getActivityAsync(TestUtils.ACTIVITY_FOR_AUTHENTICATED_USER);
			final StravaActivity activity = future.get();
