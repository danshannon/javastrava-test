...
public class UpdateActivityTest extends StravaTest {
	...
	public void testUpdateActivity_invalidActivity() throws Exception {
		RateLimitedTestRunner.run(() -> {
			...
			final StravaActivity response = stravaWithWriteAccess().updateActivity(activity.getId(),
					new StravaActivityUpdate(activity));
	...
	public void testUpdateActivity_nullUpdate() throws Exception {
		RateLimitedTestRunner
				.run(() -> {
					final StravaActivity activity = stravaWithWriteAccess().updateActivity(
							TestUtils.ACTIVITY_FOR_AUTHENTICATED_USER, null);
