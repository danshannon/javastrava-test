...
public class ListActivityLapsAsyncTest extends StravaTest {
	...
	public void testListActivityLaps_hasLaps() throws Exception {
		RateLimitedTestRunner.run(() -> {
			final List<StravaLap> laps = strava().listActivityLapsAsync(TestUtils.ACTIVITY_WITH_LAPS).get();
	...
	public void testListActivityLaps_hasNoLaps() throws Exception {
		RateLimitedTestRunner.run(() -> {
			final List<StravaLap> laps = strava().listActivityLapsAsync(TestUtils.ACTIVITY_WITHOUT_LAPS).get();

			assertNotNull("<null> list of laps returned for an activity", laps);
			assertNotEquals("Laps returned for an activity which should not have them", 0, laps.size());
			for (final StravaLap lap : laps) {
				if (lap.getResourceState() != StravaResourceState.META) {
	...
	public void testListActivityLaps_invalidActivity() throws Exception {
		RateLimitedTestRunner.run(() -> {
			final List<StravaLap> laps = strava().listActivityLapsAsync(TestUtils.ACTIVITY_INVALID).get();
	...
	public void testListActivityLaps_privateActivity() throws Exception {
		RateLimitedTestRunner.run(() -> {
			final List<StravaLap> laps = strava().listActivityLapsAsync(TestUtils.ACTIVITY_PRIVATE_OTHER_USER).get();
	...
	public void testListActivityLaps_privateActivityWithoutViewPrivate() throws Exception {
		RateLimitedTestRunner.run(() -> {
			final List<StravaLap> laps = strava().listActivityLapsAsync(TestUtils.ACTIVITY_PRIVATE_WITH_LAPS).get();
			assertNotNull(laps);
	...
	public void testListActivityLaps_privateActivityWithViewPrivate() throws Exception {
		RateLimitedTestRunner.run(() -> {
			final List<StravaLap> laps = stravaWithViewPrivate().listActivityLapsAsync(TestUtils.ACTIVITY_PRIVATE_WITH_LAPS).get();
			assertNotNull(laps);
