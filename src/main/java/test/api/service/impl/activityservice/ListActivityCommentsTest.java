...
public class ListActivityCommentsTest extends PagingListMethodTest<StravaComment, Long> {
	...
	public void testListActivityComments_activityMarkdownPaging() throws Exception {
		RateLimitedTestRunner.run(() -> {
			final List<StravaComment> comments = TestUtils.strava().listActivityComments(TestUtils.ACTIVITY_WITH_COMMENTS,
	...
	public void testListActivityComments_hasComments() throws Exception {
		RateLimitedTestRunner.run(() -> {
			...
			final List<StravaComment> commentsWithoutMarkdown = TestUtils.strava()
					.listActivityComments(TestUtils.ACTIVITY_WITH_COMMENTS, Boolean.FALSE);

			// Check that the lists are the same length!!
			assertNotNull("Returned null list of comments (without markdown) when some were expected"); //$NON-NLS-1$
			assertEquals("List of comments for activity " + TestUtils.ACTIVITY_WITH_COMMENTS //$NON-NLS-1$
	...
	public void testListActivityComments_hasNoComments() throws Exception {
		RateLimitedTestRunner.run(() -> {
			final List<StravaComment> comments = TestUtils.strava().listActivityComments(TestUtils.ACTIVITY_WITHOUT_COMMENTS,
					Boolean.TRUE);
	...
	public void testListActivityComments_invalidActivity() throws Exception {
		RateLimitedTestRunner.run(() -> {
			...
			comments = TestUtils.strava().listActivityComments(TestUtils.ACTIVITY_INVALID, Boolean.FALSE);
	...
	public void testListActivityComments_privateActivity() throws Exception {
		RateLimitedTestRunner.run(() -> {
			final List<StravaComment> comments = TestUtils.strava().listActivityComments(TestUtils.ACTIVITY_PRIVATE_OTHER_USER);
			assertNotNull(comments);
